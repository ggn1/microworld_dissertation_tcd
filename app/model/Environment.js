...
export default class Environment {
    ...
    constructor(ffEmission=null) {
         ...
         */
        this.#airMass = Big(JSON.parse(process.env.NEXT_PUBLIC_AIR_MASS))
        if (ffEmission != null) this.setFossilFuelEmission(ffEmission)
        this.carbon = {} // g
        for (const [reservoir, carbonAmount] of Object.entries(
            JSON.parse(process.env.NEXT_PUBLIC_C_START)
        )) this.carbon[reservoir] = Big(carbonAmount)
        this.land = new Land(this.updateCarbon, () => { return this.carbon }, this.getAirCO2ppm)
    }
